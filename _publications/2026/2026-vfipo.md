...
abstract_zh: >-
  Farsight 针对视觉语言模型 RLVR 后训练的一个核心失效模式：token 级信用分配只奖励「即时视觉依赖」，使模型在长推理链中逐渐脱离视觉证据。我们将信用分配扩展为「未来折扣视觉锚定」——token 获得的视觉信用不仅来自当前使用的证据，也来自该证据对折扣后未来推理步骤的支撑——从而促使策略在多步思维链中始终锚定图像。该工作完成于字节跳动（新加坡）Agentic 后训练研发实习期间。
authors:
  - Zhihui Chen
  - Yike Yun
  - et al.
